...
;;;DESCRIPTION
;;;
;;;   Originally written by Rich Hickey.
;;;
;;;   Foil 0.1
;;;
;;;    Copyright (c) 2005 Rich Hickey. All rights reserved.
;;;                  2005-2008 Rich Hickey and Christian Hofmann. All
;;;                            rights reserved.
...
;;;    Christian Hofmann <chr>
;;;
;;;    <chr> - added code for CLISP compatibility
;;;          - added logging code
...
(defun make-value-weak-hash-table ()
  #+:ccl (make-hash-table :weak t)
  #+:lispworks(make-hash-table :weak-kind :value)
  #+clisp (make-hash-table :weak :value)
...
(eval-when (:compile-toplevel :load-toplevel)
  (setf (fdefinition 'ensure-class)
        #+ccl #'ccl::ensure-class-for-defclass
        #+lispworks #'clos:ensure-class
        #+clisp #'clos:ensure-class
...
(defun make-fref (id rev &key type hash val)
  (let ((ret (make-instance 'fref :id id :rev rev :type type :hash hash :val val)))
    ...
    #+clisp (ext:finalize ret #'foil::free-fref)
...
;; chr: replaces (format send-stream "~S" args) against a version that prints
;;      all symbols besides keywords readably
(defun send-message (&rest args)
  (with-standard-io-syntax
    (let* ((send-stream (get-fvm-stream))
      ...
      (write-char #\( send-stream)
      (dolist (arg args)
	(cond
	  ((and (symbolp arg) (not (keywordp arg)) (not (stringp arg)))
	   (when +debug+ (format t "~%Sending ~A" arg))
	   (princ arg send-stream)
	   (write-char #\space send-stream))
	  (t
	   (when +debug+ (format t "~%Sending ~S" arg))
	   (prin1 arg send-stream)
	   (write-char #\space send-stream))))
      (write-char #\) send-stream)
...
;; chr: added code for CLISP that makes shure that the stream-buffer is emptied
;;      and sends a newline character after reading is completed
;;      see: "Implementation Notes for GNU CLISP Version 2.46" -
;;           Section 32.4 Socket Streams
(defun process-return-message ()
  (let* ((*readtable* *foil-readtable*)
	...
	#+CLISP(loop :for c = (read-char-no-hang ret-stream nil nil) :while c)
	(terpri ret-stream)
	(when +debug+ (format t "~%Received: ~S" msg))
         (case (first msg)
...
(defun call-prop-get (class-sym name method-sym this args)
  (with-vm-of this
    (when +debug+ (format t "prop-get ~A meth ~A~%" name method-sym))
    (let ((cref (find-prop-get class-sym name method-sym)))
      (apply #'send-message :call cref *marshalling-flags* *marshalling-depth* this args))))

(defun call-prop-set (class-sym name method-sym this args)
  (with-vm-of this
    (when +debug+ (format t "prop-set ~A meth ~A~%" name method-sym))
...
(defun dump-wrapper-defs (strm classnames)
  (let ((forms nil)
    ...
    (dolist (name (sort classnames #'string-lessp))
      (when +debug+ (format t "def-foil ~A~%" name))
      (let ((defs (do-def-foil-class name)))
        (dolist (def defs)
          (cond
	    ((eql (car def) 'ensure-package) 
             (push def ensures))
            ((eql (car def) 'export) 
             (push (second def) (gethash (third def) exports)))
            (t (push def forms))))))
